...
def test_embeddings_invalid_json():
    response = client.post("/api/v1/embeddings", data="not a json", headers={"Content-Type": "application/json"})
...
def test_embeddings_wrong_content_type():
    response = client.post("/api/v1/embeddings", data="{\"text\": \"hello\"}", headers={"Content-Type": "text/plain"})
    assert response.status_code == 422 or response.status_code == 400
