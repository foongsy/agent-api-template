...
class Settings(BaseSettings):
    ...
    langfuse_public_key: SecretStr = Field(
        default=SecretStr(""), description="Langfuse public key"
    )
    ...
    langfuse_enabled: bool = Field(
        default=True, description="Enable Langfuse tracing and monitoring"
    )
    ...
    embedding_device: str = Field(
        default="cpu", description="Device for embedding model (cpu, cuda, etc.)"
    )
    ...
    agent_temperature: float = Field(
        default=0.7, description="Temperature for agent responses (0.0 to 2.0)"
    )
    ...
    max_image_size_mb: int = Field(default=8, description="Maximum image size in MB")
