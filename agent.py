...
class AgentService:
    ...
    def _validate_images(self, images: Optional[List[bytes]], mime_types: Optional[List[str]]) -> List[BinaryContent]:
        ...
        for i, (image_bytes, mime_type) in enumerate(zip(images, mime_types)):
            ...
            if mime_type not in settings.supported_image_formats:
                raise ValueError(f"Unsupported image format: {mime_type}. Supported: {settings.supported_image_formats}")
            
            # Validate size
            if len(image_bytes) > max_size_bytes:
                raise ValueError(f"Image {i} exceeds maximum size of {settings.max_image_size_mb}MB")
            
            # Create BinaryContent for pydantic-ai
            validated_images.append(BinaryContent(data=image_bytes, media_type=mime_type))
        ...
        with langfuse_service.trace_span("agent.process_message") as span:
            try:
                ...
                if request.images:
                    ...
                    images = self._validate_images(request.images, request.image_mime_types)
                ...
                if span:
                    ...
                    truncated_response = (
                        ...
                        if len(result.output.content) > settings.langfuse_trace_content_limit 
    ...
    async def validate_agent(self) -> bool:
        ...
        try:
            ...
            if not api_key or api_key.strip() == "":
                logger.warning("No OpenRouter API key provided. Skipping live agent validation.")
