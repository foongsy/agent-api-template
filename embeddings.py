...
class EmbeddingService:
    ...
    def _setup_model(self):
        ...
        try:
            logger.info(
                f"Initializing HuggingFace embedding model: {self.model_name} on {self.device}"
            )
            ...
            raise RuntimeError(
                f"Failed to initialize embedding model {self.model_name}: {str(e)}"
            )
    ...
    async def validate_model(self):
        ...
        try:
            ...
            logger.info(
                f"Model validation successful. Embedding dimensions: {len(test_embedding)}"
            )
            return True
        except Exception as e:
    ...
    async def get_embeddings(
        ...
        try:
            ...
            if isinstance(texts, str):
                ...
                logger.debug(
                    f"Generating embedding for single text (length: {len(texts)})"
                )
            ...
            # Handle list of texts input
                ...
                for i, text in enumerate(valid_texts):
                    ...
                    logger.debug(
                        f"Generated embedding {i+1}/{len(valid_texts)} with {len(embedding)} dimensions"
                    )
